...
export default function PrevisaoDeEstoque() {
  ...
  // Deriva vendedores diretamente dos pedidos retornados pela API de logística,
  // sem depender do allowlist do módulo Metas.
  const sellers: AllowedSeller[] = useMemo(() => {
    const names = new Set<string>()
    for (const order of data?.orders ?? []) {
      if (order.sellerName) names.add(order.sellerName)
    }
    return Array.from(names)
      .sort((a, b) => a.localeCompare(b))
      .map((name) => ({ code: null, name, active: true }))
  }, [data?.orders])
  ...
  }, [data, dateFrom, dateTo, selectedSellers, selectedCities, sortedProductAggregates, stockMap, cityAggregates, groupedOrders, totals, authData, showInCarga])
