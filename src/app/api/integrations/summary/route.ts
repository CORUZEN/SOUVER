...
export async function GET(req: NextRequest) {
  ...
  const totalActive = integrations.filter((i: any) => i.status === 'ACTIVE').length
  const totalError = integrations.filter((i: any) => i.status === 'ERROR').length
  const successLogs = logStats.find((s: any) => s.status === 'success')?._count.id ?? 0
  const errorLogs = logStats.find((s: any) => s.status === 'error')?._count.id ?? 0
  const totalLogs = logStats.reduce((a: number, s: any) => a + s._count.id, 0)
