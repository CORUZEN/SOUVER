...
export default function SupervisorPwaDashboard() {
  ...
  useEffect(() => {
    ...
    const ALLOWED_SUPERVISOR_ROLES = new Set(['DIRECTORATE', 'COMMERCIAL_MANAGER', 'DEVELOPER'])
    if (!ALLOWED_SUPERVISOR_ROLES.has(roleCode)) { router.replace('/app/diretoria'); return }
  ...
  const loadData = useCallback(async () => {
    ...
    try {
      ...
      const stageW1 = weekByKey.get('W1')?.end
      const stageW2 = weekByKey.get('W2')?.end
      const stageW3 = weekByKey.get('W3')?.end
      const stageClosing = weekByKey.get('CLOSING')?.end
      if (stageW1) distParams.set('w1End', stageW1)
      if (stageW2) distParams.set('w2End', stageW2)
      if (stageW3) distParams.set('w3End', stageW3)
      if (stageClosing) distParams.set('closingEnd', stageClosing)
  ...
  const filteredSellers = useMemo(() => {
    return sellers.filter((seller) => {
      if (selectedSupervisor && seller.supervisorCode !== selectedSupervisor) return false
      if (selectedSellerCode && seller.id !== selectedSellerCode) return false
      return true
    })
  }, [sellers, selectedSupervisor, selectedSellerCode])

  // ── Derived metrics ───────────────────────────────────────────────────────
  const totalRevenue = filteredSellers.reduce((s, r) => s + r.totalValue, 0)
  const totalOrders = filteredSellers.reduce((s, r) => s + r.totalOrders, 0)
  const totalWeight = filteredSellers.reduce((s, r) => s + r.totalGrossWeight, 0)

  // ── Aggregated Focus (filtered scope) ─────────────────────────────────────
  const aggregatedFocus = useMemo(() => {
    ...
    if (focusTargetMode === 'BASE_CLIENTS') {
      const totalBase = filteredSellers.reduce((sum, s) => sum + s.baseClientCount, 0)
  ...
  }, [filteredSellers, focusConfigBySeller, focusRowsByProduct])

  // ── Aggregated Volume (filtered scope, full month) ─────────────────────────
  const aggregatedVolume = useMemo(() => {
    ...
    return { totalTarget, totalSold, pct: totalTarget > 0 ? (totalSold / totalTarget) * 100 : 0, groups }
  }, [filteredSellers, weightTargetsBySeller, brandWeightRows])

  const totalTarget = filteredSellers.reduce((s, r) => {
  ...
  const filteredSellerCards = useMemo(() => {
    return sellerCards.filter(({ seller }) => {
      if (selectedSupervisor && seller.supervisorCode !== selectedSupervisor) return false
      if (selectedSellerCode && seller.id !== selectedSellerCode) return false
      return true
    })
  }, [sellerCards, selectedSupervisor, selectedSellerCode])

  const uniqueSupervisors = useMemo(() => {
    const map = new Map<string, string>()
    for (const s of sellers) {
      if (!s.supervisorCode) continue
      if (!map.has(s.supervisorCode)) {
        const name = s.supervisorName?.trim()
        if (name) {
          map.set(s.supervisorCode, name)
        } else {
          // fallback: try to find a seller whose id matches the supervisor code
          const supervisorSeller = sellers.find(
            (x) =>
              x.id === s.supervisorCode ||
              x.id.replace(/^sankhya-/, '') === s.supervisorCode ||
              normalizeCode(x.id.replace(/^sankhya-/, '')) === normalizeCode(s.supervisorCode!)
          )
          map.set(s.supervisorCode, supervisorSeller?.name ?? s.supervisorCode)
        }
      }
    }
